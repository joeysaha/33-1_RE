  ...
  str.each_char do |char|
    ...
    if opening_chars.include?(char) # opening bracket
      ...
      if char == required_char # it's the right kind of closing bracket
        ...
        if open.any?# { |charac| charac == openables["#{char}"] } # it's the wrong kind of closing bracket
          oppo = openables[open.last]
          puts "* You have a syntax error: there is an unexpected #{char} where there should be a #{oppo}."
